...
DryHubWorld::DryHubWorld(CosmicEngine* eng)
{
	engine = eng;
	cam = new FreeCamera();
}
...
void DryHubWorld::UpdateParticlesPos()
{
...
void DryHubWorld::CheckInputs(float dt)
{
	if (!p1->isDead) {
...
void DryHubWorld::CheckControllerInputs(float dt)
{
	if (!p1->isDead) {
		bool butCheckX = false;
		bool butCheckY = false;
...
void DryHubWorld::CheckOutOfBounds()
{
...
void DryHubWorld::SetUpParticles()
{
...
void DryHubWorld::CalculateCamPos(float dt)
{
